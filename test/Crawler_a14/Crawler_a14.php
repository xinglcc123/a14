...
class Crawler extends setting
{
    ...
    public function sharksu_login($Reload = false) {
        echo $this->light_alert('登入中。。。');
        ...
        //验证码
        $url_cap = $this->setting['url'].'agent/validCode?type=agent&t=0.6917546422088694';
        echo "$url_cap";
        ...
        // 登入頁先取得Cookie值
        $data = [
             'username' => $this->setting['account'],
             'password' => $this->setting['password'],
        ];

        $res  = $this->curl_send_new($url, $data, 'GET', $this->setting['headers']);
        ...
        //         echo "[+]".PHP_EOL;
        //         var_dump($this->setting['headers']);
        //         empty($this->setting['headers'][$key]);
        //         break;
        //     }
        // }


        // preg_match("/JSESSIONID=([A-Z0-9]+);/", $res, $cookie_login_JSESSIONID);
        // preg_match('/route=(.*);/', $res, $cookie_login_route);

        // var_dump($cookie_login_JSESSIONID);
        // var_dump($cookie_login_route);
        // $value = "Cookie: JSESSIONID={$cookie_login_JSESSIONID[0]}; route={$cookie_login_route[0]}";

         if (!is_null($cookie_login_JSESSIONID[0]) && isset($cookie_login_JSESSIONID[0]) && !is_null($cookie_login_route[0]) && isset($cookie_login_route[0])) {
            array_push($this->setting['headers'], $value);
            var_dump($this->setting['headers']);
            exit;
            // var_dump($this->setting['headers']);
            $this->updateTojson($this->setting, self::SETTING_FILE);
